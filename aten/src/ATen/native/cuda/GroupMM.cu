...
// FP16 grouped GEMM implementation
template <
    typename ArchTag,
    bool a_row_major,
    bool b_row_major,
    ...
    typename TB_N,
    typename TB_K>
void f16f16_grouped_gemm_impl_sm90_sm100(
    at::Tensor mat_a, // f16
    at::Tensor mat_b, // f16
    std::optional<at::Tensor> offs,
    std::optional<at::Tensor> bias, // F16
    at::Tensor& out) {
  using DtypeA = cutlass::half_t;
  using DtypeB = cutlass::half_t;
  using DtypeOutput = cutlass::half_t;
  using DtypeAccum = float;
  using LayoutA = cute::conditional_t<
      a_row_major,
      cutlass::layout::RowMajor,
  ...
  using EpilogueSchedule =
      typename Schedule<ArchTag, PONGOr2SM, TB_M, TB_N, TB_K>::EpilogueSchedule;
  using ProblemShape = cutlass::gemm::GroupProblemShape<
      cute::Shape<int32_t, int32_t, int32_t>>; // <M,N,K> per group
...
// FP16 dispatch functions
template <bool a_row_major, bool b_row_major>
void dispatch_f16_grouped_kernel_on_tile_size(
    at::Tensor mat_a, // f16
    at::Tensor mat_b, // f16
    std::optional<at::Tensor> offs,
    std::optional<at::Tensor> bias, // F16
  ...
  if (sm10x) {
    if (small) {
      f16f16_grouped_gemm_impl_sm90_sm100<
          cutlass::arch::Sm100,
          a_row_major,
          b_row_major,
          /*PONGOr2SM*/ false,
...
void dispatch_f16_grouped_kernel_on_ab_transpose(
    at::Tensor mat_a, // f16
    at::Tensor mat_b, // f16
    std::optional<at::Tensor> offs,
    std::optional<at::Tensor> bias, // F16
  ...
  bool b_row_major = mat_b.stride(-1) == 1;
  if (a_row_major && b_row_major) {
    dispatch_f16_grouped_kernel_on_tile_size<true, true>(
        mat_a, mat_b, offs, bias, out);
  } else if (a_row_major && !b_row_major) {
    dispatch_f16_grouped_kernel_on_tile_size<true, false>(
        mat_a, mat_b, offs, bias, out);
  } else if (!a_row_major && b_row_major) {
    dispatch_f16_grouped_kernel_on_tile_size<false, true>(
        mat_a, mat_b, offs, bias, out);
  } else {
    dispatch_f16_grouped_kernel_on_tile_size<false, false>(
        mat_a, mat_b, offs, bias, out);
  }
}
...
void f16f16_grouped_mm(
    ...
    at::Tensor& out) {
#if defined(BUILD_GG_KERNEL)
  dispatch_f16_grouped_kernel_on_ab_transpose(mat_a, mat_b, offs, bias, out);
#else
  TORCH_CHECK(false, "grouped mm is not supported on your system");
#endif
}
