...
class TorchCtxManagerClassVariable(BaseTorchVariable):
    ...
    def call_function(
        ...
        if self.value is torch.no_grad:
            ...
            name = args[0].as_python_constant() if args else kwargs.get("name", variables.ConstantVariable("unknown")).as_python_constant()
            record_args = None
            if len(args) > 1:
                record_args = args[1].as_python_constant()
            elif "args" in kwargs:
                record_args = kwargs["args"].as_python_constant()
            return ProfilerContextVariable.create(name, record_args, emit_profiler_ops=True)
        elif self.value in (
            torch.profiler.profile,
            torch.autograd.profiler.profile,
        ):
            # For profile(), keep the old nullcontext-like behavior
            warning_once(log, "Profiler function %s will be ignored", self.value)
            return ProfilerContextVariable.create("profile", None, emit_profiler_ops=False)
        elif (
