...
class _CacheIntf(ABC):
    ...
    def _make_record_wrapper(
        ...
        ischema: context.IsolationSchema | None = None,
        custom_params_encoder: Callable[P, Any] | None = None,
        custom_result_encoder: Callable[[Concatenate[R, P]], Any] | None = None,
        custom_result_decoder: Callable[[Concatenate[R, Any]], R] | None = None,
    ) -> Callable[P, R]:
        pass
            ...
            try:
                ...
                if imc_get:
                    if custom_result_decoder:
                        return impls.Hit(value=custom_result_decoder(imc_get.value, params))
                    else:
                        return imc_get
                else:
                    ...
                    if odc_get:
                        if custom_result_decoder:
                            return impls.Hit(value=custom_result_decoder(odc_get.value, params))
                        return odc_get
                return None
        ...
        with locks._acquire_many_impl_locks_with_timeout(self._imc, odc):
            try:
                encoded_result: Any = (
                    ...
                    else custom_result_encoder(result, params)
                )
...
class _DeterministicCacheIntf(_CacheIntf):
    def __init__(self) -> None:
        ...
        if fpath_str := os.environ.get("TORCHINDUCTOR_PRE_POPULATE_DETERMINISTIC_CACHE"):
            fpath: Path = Path(fpath_str)
            fpath_parent: PathLike[str] = str(fpath.parent)
            if fpath.is_file():
            ...
            *([self._imc, sc] if sc else [self._imc])
            ...
            try:
                ...
                if imc_get:
                    ...
                    if sc_get:
                        if custom_result_decoder:
                            return impls.Hit(value=custom_result_decoder(sc_get.value, params))
                        return sc_get
